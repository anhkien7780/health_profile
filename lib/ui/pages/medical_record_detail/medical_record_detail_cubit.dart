...
class MedicalRecordDetailCubit extends Cubit<MedicalRecordDetailState> {
  ...
  final MedicalRecordDetailNavigator navigator;
  final int recordId;
  ...
  Future<void> checkPaymentStatusAndNavigate() async {
    final appointmentId = state.medicalRecord?.appointmentId;
    if (appointmentId == null) return;

    emit(state.copyWith(loadingStatus: LoadingStatus.loading));
    try {
      final transactions = await paymentRepository.getTransactionsForAppointment(appointmentId);
      log('Payment status check response: $transactions');
      final isPaid = transactions.any((t) => t.status == 'COMPLETED');
      emit(state.copyWith(loadingStatus: LoadingStatus.finish));

      if (isPaid) {
        navigator.navigateToPaymentConfirmed();
      } else {
